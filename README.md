...
#### I only want to try one methodology
If you only have bandwidth to try out one methodology, or trying multiple is daunting, I would recommend the LSTM for several reasons. Firstly, it is available in four different programming languages, increasing access. Secondly, it not very volatile and rarely predicts huge outliers, as can be the case with Bayesian VARs, OLS, and DFMs. This makes its predictions easier to use and count on. Third is its predictive performance, it has been shown to have high performance in multiple contexts, including extremely volatile situations; if there is a good model to be had out of your data, you are likely to get it via LSTM. Fourthly, it is very robust to the data you give it and will never fail to estimate. Bayesian VARs and DFMs frequently will fail to fit with certain data. This can greatly complicate the model development process as well as the inference process. What do you do if your selected model is no longer able to be fit or generate predictions because the latest data release is a certain way? Its biggest con is its potential need for hyperparameter tuning. Results may seem very poor at first, but may be substantially better after tuning. The `model_lstm.ipynb` file contains a recommended grid of hyperparameters to test, mitigating this problem.
...
- **Decision tree** (`model_dt.ipynb`):
	...
	- <ins>_commentary_</ins>: Simple methodology, not traditionally used in nowcasting. Doesn't handle time series, handled via including additional variables for lags. Poor performance in this benchmarking analysis. Has hyperparameters which may need to be tuned.
...
- **Long short-term memory neural network (LSTM)** (`model_lstm.ipynb`):
	...
	- <ins>_language, library, and function_</ins>: [Python, R, MATLAB, Julia], `LSTM` function of [`nowcast_lstm`](https://github.com/dhopp1/nowcast_lstm) library. Available in [Python](https://pypi.org/project/nowcast-lstm/), [R](https://github.com/dhopp1/nowcastLSTM), [MATLAB](https://github.com/dhopp1/nowcast_lstm_matlab), and [Julia](https://github.com/dhopp1/NowcastLSTM.jl).
